...
from cs336_basics.model import BasicTransformerLM, Embedding, Linear
from cs336_basics.optimizer import AdamW
from cs336_basics.nn_utils import cross_entropy
from cs336_basics.dataloader import get_batch, load_data
from cs336_systems.ddp import setup, DDP, ShardedOptimizer
...
if __name__ == "__main__":
    ...
    for name, submodule in model.named_modules():
        if isinstance(submodule, Linear) or isinstance(submodule, Embedding):
            print(f"{name}: {submodule.weight.shape}")
    
    # print model parameters and their sizes
    # for name, param in model.named_parameters():
    #     print(f"{name}: {param.shape} {param.numel() * param.element_size() / 1024 ** 2:.2f} MB")
    # for param in model.parameters():
    #     print(f"{param}: {param.shape} {param.numel() * param.element_size() / 1024 ** 2:.2f} MB")

    # benchmark optimizer with and without sharded optimizer
